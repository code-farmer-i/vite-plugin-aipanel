# Shadow write/edit：先格式化再写入

**Status: proposed**（实现未开始；3.3 已查证沙箱可公开复刻）

模型调用 write/edit 时以 prettier 格式化后的内容落盘：在 agent 作用域注册 shadow write/edit 隐藏官方实现，复用官方公开服务（`ctx.fs` / `ctx.sandboxPolicy` / `ctx.approval` / `fs/observed`）保证权限链不被绕过；方案 A（post-execute 后格式化）作为 `prettierWrite=false` 的回退。
